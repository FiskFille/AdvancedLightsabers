...
{
	private ResourceLocation texture = new ResourceLocation(Lightsabers.modid, "textures/gui/container/force_power_selector.png");
	private ResourceLocation icons = new ResourceLocation(Lightsabers.modid, "textures/gui/icons.png");
	...
	public List<PowerSlot> slots = Lists.newArrayList();
	...
	public void initGui()
	{
		super.initGui();
		slots.clear();
		selectedPowers = ALPlayerData.getData(mc.thePlayer).selectedPowers.toArray(selectedPowers);

		Power[] powers = new Power[16];
		int i = 0;

		for (Power power : ALHelper.getRelevantPowers(mc.thePlayer))
		{
			powers[i] = power;
			++i;
		}
		...
		for (int j = 0; j < 4; ++j)
		{
			for (int k = 0; k < 4; ++k)
			{
				slots.add(new PowerSlot(powers[k + j * 4], k + j * 4, 8 + k * 18, 8 + j * 18));
			}
		}
	}
			...
			if (grabbedId < 0)
			{
				for (int i = 0; i < 3; ++i)
				{
					if (selectedPowers[i] != null && new Rectangle(x + 62 + i * 18, y + 142, 16, 16).contains(mouseX, mouseY))
					{
						...
						DataManager.setSelectedPowersWithoutNotify(mc.thePlayer, Arrays.asList(selectedPowers));
					}
				}
			}
		}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		for (PowerSlot slot : slots)
		{
			...
			if (power != null)
			{
				drawTexturedModalRect(x1, y1, power.iconX * 16, power.iconY * 16, 16, 16);
		...
		for (int i = 0; i < selectedPowers.length; ++i)
		{
			...
			if (power != null && (grabX != x1 || grabY != y1 || grabPower == null))
			{
				drawTexturedModalRect(x1, y1, power.iconX * 16, power.iconY * 16, 16, 16);
			}
		...
		if (hoverPower != null)
		{
			...
			GL11.glColor4f(1, 1, 1, 1);
			ItemRenderer.renderItemIn2D(tessellator, hoverPower.iconX * 16 * f, hoverPower.iconY * 16 * f, (hoverPower.iconX * 16 + 16) * f, (hoverPower.iconY * 16 + 16) * f, 16, 16, 0);
			GL11.glDisable(GL12.GL_RESCALE_NORMAL);
			GL11.glPopMatrix();
			TextureUtil.func_147945_b();
			
			String name = hoverPower.getNameTranslated();
			int x1 = x + 8;
            int y1 = y + 84;
            ...
            {
            	String[] astring = hoverPower.powerEffect.getDesc(hoverPower.powerEffectArgs);
            ...
            {
            	fontRendererObj.drawStringWithShadow(I18n.format(stats.powerType == EnumPowerType.PER_USE ? "forcepower.perUse" : "forcepower.perSecond", ItemStack.field_111284_a.format(stats.forcePowerUseCost)), x1 + 3, height + 3, 0xa4a4a4);
		...
		{
			mc.getTextureManager().bindTexture(icons);
			drawTexturedModalRect(grabX, grabY, grabPower.iconX * 16, grabPower.iconY * 16, 16, 16);
		}
	...
	{
		DataManager.setSelectedPowers(mc.thePlayer, Arrays.asList(selectedPowers));
	}
