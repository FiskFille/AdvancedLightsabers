...
public class GuiLightsaberForge extends GuiContainer
{
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
    {
        ...
        if (itemstack != null)
        {
            float length = LightsaberHelper.getLightsaberHeightCm(itemstack);

            if (length < LightsaberHelper.MIN_LENGTH_CM)
            {
                drawTexturedModalRect(k + 131, l + 65, 176, 0, 26, 17);
                drawString(fontRendererObj, StatCollector.translateToLocal("gui.lightsaber_forge.too_short"), guiLeft + 45, guiTop + 64 - fontRendererObj.FONT_HEIGHT, 0xD74848);
            }
            else
            {
                drawString(fontRendererObj, StatCollector.translateToLocalFormatted("%s cm", ItemStack.field_111284_a.format(length)), guiLeft + 45, guiTop + 64 - fontRendererObj.FONT_HEIGHT, -1);
            }
        }
