...
public class PacketSithCoffin implements IMessage
{
    ...
    public PacketSithCoffin(Entity entity, int x, int y, int z, int action)
    {
    ...
    {
        public IMessage onMessage(PacketSithCoffin message, MessageContext ctx)
        {
            	...
            	if (tile != null)
            	{
            		...
            		if (world.getEntityByID(message.id) instanceof EntityPlayer)
            		{
            			...
            			if (action == 0)
            			{
            				...
            				{
            					tile.isLidOpen = true;
            					player.playSound(Lightsabers.modid + ":sith_sarcophagus_open", 1.0F, 1.0F);
            				}
            				else if (tile.lidOpenTimer == tile.lidOpenTimerMax)
            				{
            					tile.isLidOpen = false;
            					player.playSound(Lightsabers.modid + ":sith_sarcophagus_open", 1.0F, 0.8F);
            				}
            			}
            		}
            		...
            		{
            			ALNetworkManager.networkWrapper.sendToAll(new PacketSithCoffin(player, tile.xCoord, tile.yCoord, tile.zCoord, action));
            		}
            	}
            }
