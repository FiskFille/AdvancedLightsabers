...
public class PowerEffectMeditation extends PowerEffect
{
	...
	public String[] getDesc(Object... args)
	{
		return new String[]
		{
			PowerDesc.create("effect", PowerDesc.format("%s %s%s", PowerDesc.translateFormatted("forcepower.stat.multiply", PowerDesc.ATTACK_DAMAGE, getModifierAmount((Integer)args[0])), EnumChatFormatting.GRAY, 90), PowerDesc.CASTER),
			PowerDesc.create("to", PowerDesc.format("+%s %s", 4 + (Integer)args[0] * 2, PowerDesc.ABSORPTION), PowerDesc.CASTER)
		};
	}
