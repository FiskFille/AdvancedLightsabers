...
{
	public PowerEffectFortify()
	{
		super(Effect.fortify);
	}
	
	@Override
	public String[] getDesc(Object... args)
	{
		return new String[] {PowerDesc.create("divide", PowerDesc.FORCE_DAMAGE, getModifierAmount((Integer)args[0]))};
	}
	...
	public void render(EntityPlayer player, float partialTicks, Object... args)
	{
		...
		float f = 0.25F * (Integer)args[0];
		float f1 = (float)(player.ticksExisted + partialTicks);
		...
		for (int i = 0; i < amount; ++i)
		{
			...
			for (int j = 0; j < coverage / angleIncr; ++j)
			{
				...
				tessellator.addVertexWithUV(vec3.xCoord, vec3.yCoord, vec3.zCoord, 0, (float)j / (coverage / angleIncr));
			}
