...
public class PowerEffectChoke extends PowerEffect
{
    ...
    {
        return new String[] {PowerDesc.create("effect", PowerDesc.format("%s %s%s", 2 + (Integer) args[0] * 2, PowerDesc.DAMAGE, EnumChatFormatting.GRAY + "/"), PowerDesc.TARGET), PowerDesc.create("effect", PowerDesc.format("%s %s%s", Effect.stun, EnumChatFormatting.GRAY, getStunDuration((Integer) args[0])), PowerDesc.TARGET)};
    }
