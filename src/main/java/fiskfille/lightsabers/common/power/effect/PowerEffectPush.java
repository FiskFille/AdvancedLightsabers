...
public class PowerEffectPush extends PowerEffect
{
	...
	public String[] getDesc(Object... args)
	{
		return new String[]
		{
			PowerDesc.create("effect2", PowerDesc.format("+%s %s", getKnockback((Integer)args[0]), PowerDesc.KNOCKBACK), PowerDesc.TARGET),
			PowerDesc.create("effect2", PowerDesc.format("%s %s", getDamage((Integer)args[0]), PowerDesc.DAMAGE), PowerDesc.TARGET)
		};
	}
